...
describe('calculateProductStatistics', () => {
  it('oculta toda precisión con 0 o pocas experiencias', () => {
    expect(calculateProductStatistics([], 5)).toEqual({
      sampleSize: 0,
  ...
  it('calcula media, mediana y la distribución pública', () => {
    expect(calculateProductStatistics([2, 5, 9, 12, 18], 5)).toEqual({
      sampleSize: 5,
  ...
  it('descarta valores fuera de 1 a 60', () => {
    expect(calculateProductStatistics([0, 1, 60, 61], 2).sampleSize).toBe(2);
  });
});
